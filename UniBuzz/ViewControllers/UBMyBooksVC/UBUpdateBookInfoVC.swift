//
//  UBUpdateBookInfoVC.swift
//  UniBuzz
//
//  Created by Ahmed Durrani on 11/09/2019.
...
class UBUpdateBookInfoVC: UIViewController {
...
//    @IBOutlet weak var btnCoverType: UIButton!
    ...
    var bookinfoss : BookList?

    override func viewDidLoad() {
        super.viewDidLoad()

        titleOfCourse.text = bookinfoss?.titleOfBook
        txtAuthorName.text = bookinfoss?.author
        txtDescription.text = bookinfoss?.descriptionOfBook
        txtLocation.text =  bookinfoss?.location
        txtEdition.text = bookinfoss?.edition
        txtPrice.text  = bookinfoss?.price
        let bookCity = bookinfoss?.book_city
        let bookCoverType = bookinfoss?.cover_type_name
        let bookCoverName = bookinfoss?.book_status_name
//        btnCoverType.setTitle(bookCoverType , for: .normal)
        btnBookStatus.setTitle(bookCoverName , for: .normal)
        btnCity.setTitle(bookCity , for: .normal)
        heightOfCourseName.constant = 0.0
        txtCourseName.isHidden = true
        txtCourseCode.isHidden = true
        if bookinfoss?.courseListsss?.count == 0 {
            self.btnCourseCode.setTitle("Course Code".localized(), for: .normal)
            
        } else {
            self.btnCourseCode.setTitle("".localized(), for: .normal)
        }
        ...
        let bookStatus = bookinfoss?.book_status
        let cityIDs = bookinfoss?.city

        selectCoverType =  bookinfoss!.cover_type ?? " "
        coverStatus =  "\(bookStatus!)"
        bookLocation =  bookinfoss!.book_city!
        
        cityId =  "\(cityIDs!)"

        for (_ , obj) in ((bookinfoss?.courseListsss?.enumerated())!) {
            tagListView.addTag((obj.course?.name!)!)
            coureCode.append((obj.course?.id!)!)
        }
...
//        if bookinfoss?.image != nil {
//            mainImage.isHidden = false
//            let imageName = bookinfoss?.image
//            let imagePAth = "\(BOOKIMAGEBASEURL)\(imageName!)"
//            WAShareHelper.loadImage(urlstring: imagePAth , imageView: (mainImage!), placeHolder: "profile2")
//
//        }
        
        guard  let image = bookinfoss?.image  else   {
            return
        }
        let imagePAth = "\(BOOKIMAGEBASEURL)\(image)"
        mainImage.setImage(with: imagePAth , placeholder: UIImage(named: "profile2"))
    ...
    @IBAction func btnBookStatus_Pressed(_ sender: UIButton) {
        ...
        ActionSheetStringPicker.show(withTitle: " ".localized(), rows: allBookStatus , initialSelection: 0 , doneBlock: { [weak self ](picker, index, value) in
...
//    }
    ...
    @IBAction func btnCourseCode_Pressed(_ sender: UIButton) {
        ...
        vc?.courseListsss = bookinfoss?.courseListsss
    ...
    private   func AddBookInfo() {
        ...
        let imgUrl = self.bookinfoss!.book_cover
        self.courseCode  = coureCode.map({"\($0)"}).joined(separator: ",")
        
        let isbn = bookinfoss?.isbn
        if isSellOrDonate == 1 {
            param = [ "isbn"                             :   isbn!  ,
...
//            "cover_type"                       :  selectCoverType ,
        ...
        let idOfBook = bookinfoss?.id
        let sericeUrl = "\(UPDATEBOOK)\(idOfBook!)"
        WebServiceManager.mutliChat(params: params as Dictionary<String, AnyObject> , serviceName: sericeUrl, imageParam:"main_image", imgFileName: "main_image.png", serviceType: "", profileImage: imge  , cover_image_param: "", cover_image: nil , modelType: MessageObject.self, success: {[weak self] (response) in
            SVProgressHUD.dismiss()
            guard let this = self  else {
                return
            }
...
//                self?.navigationController?.popToRootViewController(animated: true)
            ...
            {
                this.showAlert(title: KMessageTitle, message: post!.message!, controller: self)
            }
        }) { [weak self] (error) in
        }
    }
...
extension UBUpdateBookInfoVC : PhotoTweaksViewControllerDelegate {
...
extension UBUpdateBookInfoVC : TagListViewDelegate {
...
extension UBUpdateBookInfoVC  : CourseSelectDelegate {
