...
class UBBookStoreVC: UIViewController {
    ...
    func getAllBooks() {
        WebServiceManager.get(params: nil, serviceName: BOOKSTORE , serviceType: "Book List".localized(), modelType: Book.self, success: {[weak self] (response) in
    ...
    func makeRequest(pageSize : Int)  {
        ...
        serviceURL = "\(BOOKSTORE)?page=\(page)"
    ...
    func getSearchResult(search : String) {
        ...
        let serviceUrl = "\(BOOKSTORE)?q=\(allSearch)"
...
extension UBBookStoreVC : UITableViewDelegate , UITableViewDataSource {
...
extension UBBookStoreVC : UISearchBarDelegate {
